...
export const VerificationFlow = ({ onComplete, existingRegistration }: VerificationFlowProps) => {
    ...
    if (existingRegistration) {
      setSteps(prev => prev.map(step => {
        switch (step.id) {
          ...
          case 'twitter-follow':
            return { ...step, completed: existingRegistration.twitter_followed };
          case 'telegram-join':
            return { ...step, completed: existingRegistration.telegram_joined };
          case 'tweet':
            return { ...step, completed: existingRegistration.tweet_verified };
          case 'registration':
            return { ...step, completed: existingRegistration.social_verified };
          default:
            return step;
        }
      }));
...
Get your tokens: ${window.location.href}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="cyber-border bg-cyber-dark/50 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="text-cyber-neon">Verification Progress</CardTitle>
          <CardDescription className="text-cyber-light/60">
            ...
            <>
              <Alert className="border-cyber-neon/20 bg-cyber-darker/50">
                ...
                <AlertDescription className="text-cyber-light/80">
                  <strong>Requirements:</strong> Use hashtags #NimRev #VERM #GridSecurity and invite 1-3 friends
                </AlertDescription>
              </Alert>

              <Button
                onClick={() => window.open(`https://twitter.com/intent/tweet?text=${generateTweetText()}`, '_blank')}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, friendsInvited: parseInt(e.target.value) || 1 })}
                  className="bg-cyber-darker border-cyber-neon/20 text-cyber-light"
                />
              </div>
